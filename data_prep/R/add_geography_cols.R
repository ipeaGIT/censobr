...
add_geography_cols <- function(arrw, year){
  ...
  # 1960: impute NA to code_muni
  if (year == 1960) {
  all_code_muni <- geobr::read_municipality(year = 1960)$code_muni
    arrw <- mutate(arrw,
                   code_muni = if_else(code_muni %in% all_code_muni, code_muni, NA))
  }
  ...
  # state name
  col <- case_when(year == 1960 ~ 'uf',
                   year == 1970 ~ 'code_state',
                   year == 1980 ~ 'V2',
                   year == 1991 ~ 'V1101',
                   year == 2010 ~ 'V0001',
                   year == 2000 ~ 'V0102')
  ...
  # 1960: MS pertence a MT , e TO pertence a GO
  if(year == 1960){
    arrw <- mutate(arrw,
                   name_state_1960 = case_when(
                     code_state == 11 ~'Rondônia',
                     code_state == 12 ~'Acre',
                     code_state == 13 ~'Amazonas',
                     code_state == 14 ~'Roraima',
