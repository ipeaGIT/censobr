...
In this example we'll be calculating the proportion of people with higher education in different racial groups in the state of São Paulo. First, we need to use the `read_population()` function to download the of population.
...
Now, we use the `dplyr` syntax to (a) filter observations for the state of Rio de Janeiro, (b) group observations by racial group, (c) summarize the data calculating the proportion of individuals with higher education.
...
df <- pop |>
      ...
      collect() |>
      group_by(V0606) |>                                                  # (b)
      summarize(higher_edu = sum(V0010[which(V6400==4)]) / sum(V0010),    # (c)
                pop = sum(V0010) ) |>
      collect()
