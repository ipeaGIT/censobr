...
All of the data aggregated at census tracts are organized followig the same logic.

In the cases when there are multiple files in the same dataset, we add a reference to the number of the file as a prefix to the variable name. To illustrate this, let's have a look at the `"Domicilio"` dataset, for example. This dataset is based on two separate tables: *Domicilio01* and *Domicilio02*. So the names of the columns in this dataset are organized as follows.
...
To check the meaning of each variable, users can run the `data_dictionary()`, which will open on the browser a `.pdf` file with the dictionary of variables in each dataset
...
In these examples below, example we'll use the city of Belo Horizonte for demonstration purpouses. So we can start by downloading the the geometries of the census tracts in the area. First, we need to download the geometries of all census tracts in the state of Minas Gerais (MG), and then keep only the ones in the municipality of Belo Horizonte. We'll also download the muncipality borders of BH.

```{r, eval = use_suggested_pkgs, warning = FALSE}
muni_bh <- geobr::read_municipality(code_muni = 'MG', year = 2010) |>
