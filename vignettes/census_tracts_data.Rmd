...
At the moment, this function only includes data from the 2010 and 2022 censuses.
...
# ---- data ---------------------------------------------------------------
dic_2010 <- c(
  "`Basico`", "`Entorno`", "`Domicilio`", "`DomicilioRenda`",
  "`Pessoa`", "`PessoaRenda`", "`Responsavel`", "`ResponsavelRenda`"
)

dic_2022 <- c(
  "`Basico`", "`Domicilio`", "`Entorno`", "`Indigenas`",
  "`Quilombolas`", "`Obitos`", "`Pessoas`", "`ResponsavelRenda`",
  "`Preliminares`"
)

# pad the shorter vector with blanks so the columns have equal length
max_len <- max(length(dic_2010), length(dic_2022))
dic_2010 <- c(dic_2010, rep("", max_len - length(dic_2010)))
dic_2022 <- c(dic_2022, rep("", max_len - length(dic_2022)))

tab <- data.frame(`2010` = dic_2010,
                  `2022` = dic_2022,
                  check.names = FALSE)
