...
Because of the seaminless integration between {`arrow`} and  {`dplyr`} , Arrow tables can be analyzed like a regular `data.frame` using the {`dplyr`} syntax. Note that we need to add a `collect()` call at the end of our query to retrieve data into a local tibble.
