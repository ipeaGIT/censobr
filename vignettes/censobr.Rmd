...
In this example, we are going to map how much people spend with rent across different municipalities in Brazil. First, we can easily download the households data set with the `read_households()` function.
...
Now we're going to (a) group observations by municipality, (b) calculate the average rent, and (c) collect the results.
...
head(rent)
```
In order to create a map with of these values, we are going to use the `geobr` package to download the geometries of Brazilian municipalities.
...
rent_sf <- left_join(muni_sf, rent, by = 'code_muni')
