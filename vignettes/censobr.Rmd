...
It also includes a couple support functions:

6. `data_dictionary()` to open the data dictionary of variables
7. `censobr_cache()` to manage the data chached locally.
...
class(pop)
```
By default, the ouput of the function is an `"arrow_dplyr_query"`. This is makes it possible for you to work with the census data in a super fast and efficient way, even though the data set might be to big for your computer memory.

> By setting the parameter `as_data_frame = TRUE`, the read functions load the entire output to memory as a `data.frame`. *Warning:* This can cause the R session to crash in computationally constrained enviroments.

The output of the read functions in **censobr** can be analyzed like a regular `data.frame` using the `{dplyr}` package. For example, it's to have a quick peak into the data with `glimpse()`
